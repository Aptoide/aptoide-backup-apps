...
public class InstalledAdapter extends MultiChoiceBaseAdapter {
  ...
  public InstalledAdapter(Bundle savedInstance, Context context, int resource,
      ArrayList<InstalledApk> objects, ArrayList<Integer> backedupPackages,
  ...
  @Override public View getViewImpl(int position, View convertView, ViewGroup parent) {
        ...
        if (sPrefs.contains(Constants.LOGIN_USER_LOGIN)) {
          ...
          for (Long id : items) {
            if (!DownloadManager.INSTANCE.mIds.contains(id)) {
              ...
              UploadFiles files =
                  new UploadFiles(apk.getApkFile(), apk.getMainObbFile(), apk.getPatchObbFile());
              ApkMetaData metaData = new ApkMetaData();
              metaData.setName(apk.getName());
              metaData.setPackageName(apk.getPackageName());

              UploadModel model =
                  new UploadModel(files, metaData, sPrefs.getString(Constants.LOGIN_USER_TOKEN, ""),
                      sPrefs.getString(Constants.LOGIN_USER_DEFAULT_REPO, ""));
              DownloadInfo info = DownloadManager.INSTANCE.getDownloadInfo(null,
          ...
          Toast.makeText(getContext(), R.string.backup_message_login_to_backup, Toast.LENGTH_LONG)
              .show();
        ...
        for (Long id : items) {
          ...
          Intent intent = new Intent(Intent.ACTION_DELETE, Uri.fromParts("package",
              context.getPackageManager()
                  .getPackageArchiveInfo(apk.getPath(), 0).packageName, null));
  ...
  @Override public InstalledApk getItem(int position) {
    return objects.get(
        position);  //To change body of implemented methods use File | Settings | File Templates.
  }
