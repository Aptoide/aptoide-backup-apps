...
public class SubmitForm extends BaseSherlockActivity {
  ...
  @Override public boolean onOptionsItemSelected(MenuItem item) {
    switch (item.getItemId()) {
      case R.id.submit:
        info.getUploadModel()
            .getMetadata()
            .setDescription(form_desc.getText()
                .toString());
        info.getUploadModel()
            .getMetadata()
            .setApk_phone(form_phone.getText()
                .toString());
        info.getUploadModel()
            .getMetadata()
            .setApk_email(form_e_mail.getText()
                .toString());
        info.getUploadModel()
            .getMetadata()
            .setApk_website(form_url.getText()
                .toString());

        info.getUploadModel()
            .getMetadata()
            .setCategory(form_category.getAdapter()
                .getItemId(form_category.getSelectedItemPosition()));
        info.getUploadModel()
            .getMetadata()
            .setRating(EnumRating.values()[form_rating.getSelectedItemPosition() + 1]);
  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
    ...
    form_name.setText(info.getApk()
        .getName());
    if (info.getUploadModel()
        .getMetadata()
        .getDescription() != null) {
      form_desc.setText(info.getUploadModel()
          .getMetadata()
          .getDescription());
    }

    String repo = PreferenceManager.getDefaultSharedPreferences(this)
        .getString(Constants.LOGIN_USER_DEFAULT_REPO, "");

    String rating = info.getUploadModel()
        .getMetadata()
        .getRating();
    ...
    long category = info.getUploadModel()
        .getMetadata()
        .getCategory();
    ...
    String phoneNumber = info.getUploadModel()
        .getMetadata()
        .getApk_phone();
    ...
    String email = info.getUploadModel()
        .getMetadata()
        .getApk_email();
    ...
    String websiteUrl = info.getUploadModel()
        .getMetadata()
        .getApk_website();
    ...
    ArrayList<EnumUploadFailReason> errorMessage = new ArrayList<EnumUploadFailReason>(
  ...
  static class GetCategories extends AsyncTask<String, Void, ArrayList<Category>> {
    ...
    public GetCategories(Spinner form_category, long category, BaseAdapter adapter,
    ...
    @Override protected ArrayList<Category> doInBackground(String... params) {
      ...
      try {
        ...
        String json =
            getJSON("http://webservices.aptoide.com/webservices/2/listCategories/json", 5000);
        ...
        for (int i = 0; i < standardCategories.length(); i++) {
          long id = standardCategories.getJSONObject(i)
              .getLong("id");
          String name = standardCategories.getJSONObject(i)
              .getString("name");
          if (!standardCategories.getJSONObject(i)
              .isNull("parent")) {
    ...
    @Override protected void onPostExecute(ArrayList<Category> categories) {
      ...
      if (categories != null) {
        ...
        form_category.setSelection(
            ((CategoriesSpinnerAdapter) adapter).getPositionFromId(category));
      }
    }
  }
  ...
  private class CategoriesSpinnerAdapter extends ArrayAdapter<Category> {
    ...
    public int getPositionFromId(long category) {
      ...
      for (int i = 0; i < getCount(); i++) {
        if (getItemId(i) == category) return i;
      }
