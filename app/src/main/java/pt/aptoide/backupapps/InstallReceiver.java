...
public class InstallReceiver extends BroadcastReceiver {
  @Override public void onReceive(Context context, Intent intent) {
    ...
    Database.getInstance()
        .removeInstalledApk(intent.getData()
            .getEncodedSchemeSpecificPart());
    BusProvider.getInstance()
        .post(new PackagesChangedEvent());

    if (intent.getAction()
        .equals(Intent.ACTION_PACKAGE_ADDED)) {
      if (sPref.getBoolean("automatic_install", false) && UploadPermissions.isUploadPermited(
          context)) {
        try {
          InstalledApk apk = InstalledAppsHelper.getInstalledApk(context, intent.getData()
              .getEncodedSchemeSpecificPart());
          UploadFiles files =
          ...
          UploadModel model =
              new UploadModel(files, metaData, sPref.getString(Constants.LOGIN_USER_TOKEN, ""),
                  sPref.getString(Constants.LOGIN_USER_DEFAULT_REPO, ""));
          DownloadInfo info =
              new DownloadInfo(new Random().nextInt(Integer.MAX_VALUE), apk, context);
