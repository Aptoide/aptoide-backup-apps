...
public class MainService extends Service {
            ...
            switch (responseCode) {
                ...
                case 401:
                    ...
                    if(server.getLogin() != null && !server.getLogin().getPassword().equals("")) {
                        is = NetworkUtils.getInputStream(urlString, server.getLogin().getUsername(), server.getLogin().getPassword(), getApplicationContext());
                    } else {
    ...
    public InstalledApk getInstalledApk(String auto_backup) {
        ...
        for(InstalledApk apk : installedApks){
            if(apk.getPackageName().equals(auto_backup)){
                return apk;
            }
        }
