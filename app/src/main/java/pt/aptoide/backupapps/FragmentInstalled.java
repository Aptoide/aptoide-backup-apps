...
public class FragmentInstalled extends SherlockListFragment {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        installedAdapter = new InstalledAdapter(savedInstanceState, getSherlockActivity(), 0, installedApks, backedUpApps);
    ...
    @Subscribe
    public void onRefreshBackedUpApps(RefreshInstalledBackedUpAppsEvent event){
        backedUpApps.clear();
    ...
    @Subscribe
    public void onRefreshInstalledApps(RefreshInstalledAppsEvent event){
        installedApks.clear();
        if(!event.isSystemApps()){
            for (Iterator<InstalledApk> iterator = event.getInstalledApks().iterator(); iterator.hasNext(); ) {
    ...
    public void onLoginEvent(LoginEvent event){
        ...
        if(!installedAdapter.getLocalCheckedItems().isEmpty()){
        ...
        @Subscribe
    public void onActionModeFinished(ActionModeFinishEvent event){
