...
public class DownloadManager {
  ...
  static int notification_icon = Build.VERSION.SDK_INT <= 8 ? R.drawable.ic_notification
      : R.drawable.ic_backup_apps_notification;
  ...
  private DownloadManager() {
    ...
    BusProvider.getInstance()
        .register(this);
    manager = (NotificationManager) BackupAppsApplication.getContext()
        .getSystemService(Context.NOTIFICATION_SERVICE);
  }

  public void removeAllActiveDownloads() {
    while (mOngoingList.iterator()
        .hasNext()) {
      mOngoingList.get(0)
          .pause();
    }
  ...
  private void refreshNotification() {
    ...
    if (mOngoingList.isEmpty()) {
      ...
      PendingIntent pIntent =
          PendingIntent.getActivity(BackupAppsApplication.getContext(), 0, intent, 0);

      notification =
          new NotificationCompat.Builder(BackupAppsApplication.getContext()).setContentTitle(
              BackupAppsApplication.getContext()
                  .getString(R.string.app_name))
              .setContentText(mOngoingList.size() + " " + BackupAppsApplication.getContext()
                  .getString(R.string.download_notification_short_on_going))
              .setSmallIcon(notification_icon)
              .setLargeIcon(BitmapFactory.decodeResource(BackupAppsApplication.getContext()
                  .getResources(), R.mipmap.backup))
  ...
  @Subscribe public void onDownloadStatus(DownloadStatusEvent event) {
    Log.d("TAG", "onDownloadStatus");
    updatePendingList();
  }

  @Subscribe public void onUploadStatus(UploadStatusEvent event) {
  ...
  public void removeDownload(DownloadInfo downloadInfo) {
    ...
    //		saveDownloadsFile();
    BusProvider.getInstance()
        .post(new DownloadRemoveEvent(downloadInfo.getId()));
  }
