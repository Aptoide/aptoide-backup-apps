...
public class DownloadExecutorImpl implements DownloadExecutor {
  ...
  public static boolean canRunRootCommands() {
    ...
    try {
      ...
      retval = false;
      Log.d("ROOT", "Root access rejected [" + e.getClass()
  ...
  @Override public void execute(final String path, final Apk apk) {
    ...
    if (canRunRootCommands() && root) {
      ...
      try {
        ...
        onClick.setDataAndType(Uri.fromFile(new File(path)),
        ...
        mBuilder.setContentTitle("Aptoide Backup Apps")
            .setContentText(
                context.getString(R.string.notification_message_installing, apk.getName()));

        Bitmap bm = BitmapFactory.decodeFile(ImageLoader.getInstance()
            .getDiscCache()
            .get(apk.getPackageName() + "|" + apk.getVersionCode())
            .getAbsolutePath());
        ...
        new Thread(new Runnable() {
          @Override public void run() {
            try {
              ...
              Log.d("TAG",
                  failure + " " + p.exitValue() + " " + apk.getPackageName() + " " + apk.getId());
              if (p.exitValue() != 255 && !failure.toLowerCase(Locale.ENGLISH)
                  .contains("failure") && !failure.toLowerCase(Locale.ENGLISH)
                  .contains("segmentation")) {
                // Sucess :-)

                mBuilder = new NotificationCompat.Builder(context);

                Intent onClick = context.getPackageManager()
                ...
                if (onClick == null) {
                  ...
                  try {
                    context.getPackageManager()
                        .getPackageInfo(apk.getPackageName(), 0);
                ...
                mBuilder.setContentTitle(context.getString(R.string.app_name))
                    .setContentText(
                        context.getString(R.string.notification_message_installed, apk.getName()));
