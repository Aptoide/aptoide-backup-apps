...
public class AutoUpdate extends AsyncTask<Void, Void, AutoUpdate.AutoUpdateInfo> {
  ...
  private static final String TMP_UPDATE_FILE = Environment.getExternalStorageDirectory()
      .getPath() + "/.aptoide/aptoideUpdate.apk";
  ...
  private void requestUpdateSelf(final AutoUpdateInfo autoUpdateInfo) {
    ...
    updateSelfDialog.setMessage(
        activity.getString(R.string.update_self_message, "Aptoide Backup Apps"));
    updateSelfDialog.setCancelable(false);
    updateSelfDialog.setButton(Dialog.BUTTON_POSITIVE, activity.getString(android.R.string.yes),
    ...
    if (autoUpdateInfo != null) {
      ...
      try {
        if (vercode > activity.getPackageManager()
  ...
  private class DownloadSelfUpdate extends AsyncTask<AutoUpdateInfo, Void, Void> {
    ...
    @Override protected Void doInBackground(AutoUpdateInfo... paramArrayOfParams) {
      try {
        ...
        if (mHttpResponse.getStatusLine()
          ...
          InputStream getit = mHttpResponse.getEntity()
              .getContent();
