...
public class AccountRemovalWatcherReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        boolean notLoggedInAccountManager = accountManager.getAccountsByType("cm.aptoide.pt").length == 0;
        ...
        if(notLoggedInAccountManager) {
            if(sPref.getBoolean(Constants.LOGGED_FROM_ACCOUNT_MANAGER, false)) {
                ...
                BusProvider.getInstance().post(new LogoutEvent().setFromAccountManager(true));
            }
        }
    }
}
